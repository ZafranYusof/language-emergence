...
if (typeof document !== 'undefined' && !document.getElementById(styleId)) {
  ...
  el.textContent = `
    ...
    @keyframes am-cursor-blink {
      0%, 100% { border-right-color: transparent; }
      50% { border-right-color: currentColor; }
    }
    @keyframes am-radar-transition {
      from { opacity: 0.5; }
      to { opacity: 1; }
    }
    @keyframes am-drift-pulse {
      0%, 100% { opacity: 0.6; }
      50% { opacity: 1; }
    }
  `;
...
function useSyntheticThoughts(agent, interval = [3000, 5000]) {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      idx.current++;
      setThoughts(prev => {
        const next = [{ ...t, id: Date.now() + Math.random(), timestamp: Date.now() }, ...prev];
  ...
  return (
    <div style={{ flex: 1, minWidth: 0 }}>
      <div style={{
        ...
        letterSpacing: 2, marginBottom: 10, display: 'flex', alignItems: 'center', gap: 6,
      }}>
        {icon} {label} THOUGHTS
      </div>
      <div style={{ maxHeight: 260, overflowY: 'auto', paddingRight: 4 }}>
        {thoughts.length === 0 ? (
          <div style={{ color: C.dim, fontSize: 10, textAlign: 'center', padding: 20 }}>
          ...
          thoughts.map((t, i) => (
            <div key={t.id} style={{
              ...
              <span style={{ fontSize: 16, lineHeight: '18px', flexShrink: 0 }}>
                {t.icon || t.emotion}
              </span>
              <div style={{ flex: 1, minWidth: 0 }}>
                {i === 0 ? (
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setHistory(prev => {
        ...
        const point = {
          t: now,
          speaker: emotionToValues(sEmo),
          listener: emotionToValues(lEmo),
        };
...
function EmotionOscilloscope({ speaker, listener }) {
  ...
  useEffect(() => {
    ...
    const sData = history.map(p => p.speaker[selectedEmotion] || 0);
    const lData = history.map(p => p.listener[selectedEmotion] || 0);

    drawLine(sData, C.cyan);
    drawLine(lData, C.amber);

  }, [history, selectedEmotion]);

  return (
    <div style={{
      background: C.panel, border: `1px solid ${C.cyan}22`, borderRadius: 12,
      padding: 16, marginBottom: 28, animation: 'am-fadeIn 0.6s ease-out',
    }}>
      <div style={{
        fontSize: 11, color: C.cyan, letterSpacing: 3, marginBottom: 12,
        borderBottom: `1px solid ${C.cyan}22`, paddingBottom: 8,
      ...
      <canvas
        ...
        style={{
          ...
          background: `${C.bg}88`, border: `1px solid ${C.dim}22`,
        }}
      />
    </div>
  );
...
function PersonalityRadar({ personality, color, size = 160, initialPersonality, showDrift = false }) {
  ...
  // Drift calculation
  const drift = initialPersonality ? traits.reduce((sum, t) => {
    return sum + Math.abs((personality?.[t] || 0) - (initialPersonality?.[t] || 0));
  }, 0) / traits.length : 0;

  return (
    <div style={{ position: 'relative' }}>
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}
        style={{ display: 'block', margin: '0 auto', animation: 'am-radar-transition 0.8s ease' }}>
        {/* grid rings */}
        {gridLevels.map((lv) => {
        ...
        {traits.map((t, i) => {
          ...
          return (
            <g key={t}>
              ...
              <text x={lx} y={ly} fill={C.dim} fontSize={8} textAnchor="middle" dominantBaseline="middle"
                fontFamily="JetBrains Mono, monospace">
                {t.slice(0, 4)}
              </text>
            </g>
...
function CircularGauge({ value, size = 100, strokeWidth = 8, color = C.green, label, sublabel, animate = false }) {
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}
      style={{ display: 'block', margin: '0 auto', animation: animate ? 'am-glow 3s ease-in-out infinite' : undefined }}>
      {/* track */}
      ...
      {/* center text */}
      <text x={cx} y={sublabel ? cy - 4 : cy + 2} textAnchor="middle" fill={color}
        fontSize={size > 80 ? 18 : 13} fontFamily="JetBrains Mono, monospace" fontWeight="bold">
        {pct(value)}
      </text>
      {sublabel && (
...
function EmotionBar({ emotion, color }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 6 }}>
      <span style={{ fontSize: 20, animation: 'am-pulse 2s ease-in-out infinite' }}>
        {emotion?.emoji || '😐'}
      </span>
      <div style={{ flex: 1 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3 }}>
          <span style={{ color: C.textBright, fontSize: 11, fontFamily: 'JetBrains Mono, monospace', fontWeight: 'bold' }}>
            {(emotion?.emotion || 'NEUTRAL').replace(/_/g, ' ')}
          </span>
          <span style={{ color: C.dim, fontSize: 10, fontFamily: 'JetBrains Mono, monospace' }}>
            {pct(intensity)}
          </span>
        </div>
        <div style={{ height: 6, background: C.panelLight, borderRadius: 3, overflow: 'hidden' }}>
          <div style={{
            height: '100%', width: pct(intensity), borderRadius: 3,
...
function RelationshipMeter({ relationship }) {
  ...
  return (
    <div style={{
      background: `linear-gradient(135deg, ${C.panel}, ${C.panelLight})`,
      border: `1px solid ${barColor}22`, borderRadius: 14, padding: 24,
      marginBottom: 28, animation: 'am-fadeIn 0.7s ease-out',
    }}>
      <div style={{
        fontSize: 11, color: C.cyan, letterSpacing: 3, marginBottom: 20,
        borderBottom: `1px solid ${C.cyan}22`, paddingBottom: 8, textAlign: 'center',
      }}>
        ◈ RELATIONSHIP METER ◈
      </div>
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <div style={{
          ...
          background: `linear-gradient(90deg, ${C.red}, ${C.amber}88, ${C.green})`,
        ...
        <div style={{
          ...
          color: barColor, letterSpacing: 3,
          textShadow: `0 0 10px ${barColor}44`,
...
function RelMetric({ label, value, color }) {
  return (
    <div style={{
      background: C.panelLight, borderRadius: 8, padding: '8px 14px',
      border: `1px solid ${color}22`, textAlign: 'center', minWidth: 100,
    }}>
      <div style={{ fontSize: 8, color: C.dim, fontFamily: 'JetBrains Mono, monospace', letterSpacing: 1 }}>
        {label}
      </div>
      <div style={{
        fontSize: 16, fontWeight: 'bold', color, fontFamily: 'JetBrains Mono, monospace',
        marginTop: 2, textShadow: `0 0 8px ${color}33`,
      }}>
        {pct(value)}
      </div>
      <div style={{ height: 3, background: `${C.dim}22`, borderRadius: 2, marginTop: 4 }}>
        <div style={{
          height: '100%', width: pct(value), borderRadius: 2,
...
function MemoryPalace({ speaker, listener }) {
  ...
  const buildMemories = (agent, agentName) => {
    ...
    for (let i = 0; i < nodeCount; i++) {
      ...
      nodes.push({
        id: `${agentName}-${i}`,
        type: isFailure ? 'failure' : isSuccess ? 'success' : 'neutral',
        importance: 0.3 + (seed / 100) * 0.7,
        content: isFailure
          ? `Failed interaction #${i + 1}: Communication breakdown`
          : isSuccess
            ? `Successful exchange #${i + 1}: High reward received`
            : `Interaction #${i + 1}: Standard exchange`,
        agent: agentName,
      });
    }

    // Add preferred symbol memories
    (mem.preferred_symbols || []).forEach((s, i) => {
      nodes.push({
        id: `${agentName}-sym-${i}`,
        type: 'success',
        importance: 0.8,
        content: `Preferred symbol: ${s}`,
        agent: agentName,
      });
    });
  ...
  const allMemories = [
    ...buildMemories(speaker, 'speaker'),
    ...buildMemories(listener, 'listener'),
  ];
  ...
  return (
    <div style={{
      background: C.panel, border: `1px solid ${C.purple}22`, borderRadius: 14,
      padding: 20, marginBottom: 28, animation: 'am-fadeIn 0.6s ease-out',
    }}>
      <div style={{
        fontSize: 11, color: C.purple, letterSpacing: 3, marginBottom: 16,
        borderBottom: `1px solid ${C.purple}22`, paddingBottom: 8, textAlign: 'center',
      }}>
        ◈ MEMORY PALACE ◈
      </div>
      ...
      <div style={{
        ...
        <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          <span style={{ width: 8, height: 8, borderRadius: '50%', background: C.green }} /> Success
        </span>
        <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          <span style={{ width: 8, height: 8, borderRadius: '50%', background: C.red }} /> Failure
        </span>
        <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          <span style={{ width: 8, height: 8, borderRadius: '50%', background: C.dim }} /> Neutral
        </span>
      </div>
          ...
          return (
            <div
              ...
              style={{
                ...
                background: color,
                opacity: mem.agent === 'speaker' ? 0.8 : 0.6,
                cursor: 'pointer',
...
function AgentCard({ agent, name, icon, color }) {
  ...
  return (
    <div style={{
      flex: 1, minWidth: 280,
      background: `linear-gradient(135deg, ${C.panel}, ${C.panelLight})`,
      border: `1px solid ${color}33`, borderRadius: 12, padding: 20,
      animation: 'am-fadeIn 0.6s ease-out',
    }}>
      {/* header */}
      <div style={{
        display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16,
        borderBottom: `1px solid ${color}22`, paddingBottom: 12,
      }}>
        <span style={{
        ...
        <span style={{
          ...
          color: C.bg, background: color, padding: '2px 8px', borderRadius: 4, fontWeight: 'bold',
        }}>
          {personality?.dominant_trait?.toUpperCase() || 'UNKNOWN'}
        </span>
      </div>

      {/* radar with drift */}
      <PersonalityRadar
        personality={radarPersonality}
        color={color}
        size={170}
        initialPersonality={initialPersonality.current}
        showDrift={true}
      />
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, margin: '10px 0' }}>
        {traits.map((t) => (
          <div key={t} style={{
            flex: '1 0 45%', display: 'flex', justifyContent: 'space-between',
            fontSize: 9, fontFamily: 'JetBrains Mono, monospace', color: C.dim, padding: '2px 4px',
            background: `${C.panelLight}88`, borderRadius: 3,
          }}>
            <span>{t.slice(0, 4)}</span>
            <span style={{ color }}>{pct(radarPersonality[t])}</span>
          </div>
        ))}
      </div>
      ...
      <div style={{ marginTop: 12 }}>
        <div style={{ fontSize: 9, color: C.dim, fontFamily: 'JetBrains Mono, monospace', marginBottom: 4, letterSpacing: 1 }}>
          CURRENT EMOTION
        </div>
      ...
      <div style={{ marginTop: 16 }}>
        <div style={{ fontSize: 9, color: C.dim, fontFamily: 'JetBrains Mono, monospace', marginBottom: 8, letterSpacing: 1 }}>
          MEMORY BANK
        </div>
...
function StatMini({ label, value, color }) {
  return (
    <div style={{
      background: C.panelLight, borderRadius: 6, padding: '6px 8px',
      border: `1px solid ${C.dim}22`,
    }}>
      <div style={{ fontSize: 8, color: C.dim, fontFamily: 'JetBrains Mono, monospace', letterSpacing: 0.5 }}>
        {label}
      </div>
      <div style={{ fontSize: 13, color, fontFamily: 'JetBrains Mono, monospace', fontWeight: 'bold', marginTop: 2 }}>
        {value}
      </div>
    </div>
  );
...
function ThoughtBubble({ thought, agentColor }) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        flex: 1, background: `${color}10`, border: `1px solid ${color}33`,
        ...
        <div style={{
          ...
          fontSize: 8, fontFamily: 'JetBrains Mono, monospace',
          color: C.bg, background: color, padding: '1px 6px',
          borderRadius: '0 0 6px 0', fontWeight: 'bold',
        }}>
          EP.{thought.episode ?? '?'}
        </div>
        <div style={{
          ...
          animation: 'am-typewriter 2s steps(60,end) forwards',
        }}>
          {thought.content}
        </div>
        <div style={{
          fontSize: 8, fontFamily: 'JetBrains Mono, monospace',
          color: C.dim, marginTop: 4, textTransform: 'uppercase', letterSpacing: 1,
        }}>
...
function JudgmentCard({ judgment, judgeColor, judgeLabel }) {
  ...
  return (
    <div style={{
      background: C.panel, border: `1px solid ${qColor}33`, borderRadius: 10,
      padding: 16, flex: 1, minWidth: 240,
      ...
      <div style={{
        ...
        {judgeLabel} JUDGES {judgment.target?.toUpperCase() || 'OTHER'}
      </div>

      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
        <span style={{
          fontSize: 12, fontFamily: 'JetBrains Mono, monospace', fontWeight: 'bold',
          color: qColor, background: `${qColor}15`, padding: '3px 10px', borderRadius: 6,
          border: `1px solid ${qColor}33`,
        }}>
          {qualityLabel(judgment.quality)}
        </span>
        <span style={{ color: C.amber, fontSize: 14, letterSpacing: 2 }}>
          {stars(judgment.score)}
        </span>
      </div>
      ...
      <div style={{
        ...
        "{judgment.reason || 'No reason given'}"
      </div>

      <CircularGauge
        value={clamp01(judgment.score)}
        size={80}
        strokeWidth={6}
        color={qColor}
        sublabel="SCORE"
...
function RelationshipMap({ relationship }) {
  ...
  return (
    <div style={{
      background: `linear-gradient(135deg, ${C.panel}, ${C.panelLight})`,
      border: `1px solid ${C.cyan}22`, borderRadius: 14, padding: 24,
      textAlign: 'center', animation: 'am-fadeIn 0.7s ease-out',
    }}>
      <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 20, flexWrap: 'wrap' }}>
        ...
        <div style={{ textAlign: 'center' }}>
          <div style={{
            width: 60, height: 60, borderRadius: '50%', border: `2px solid ${C.cyan}`,
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            fontSize: 24, background: `${C.cyan}15`, margin: '0 auto',
          }}>
            🔵
          </div>
          <div style={{ fontSize: 9, fontFamily: 'JetBrains Mono, monospace', color: C.cyan, marginTop: 6 }}>
            SPEAKER
          </div>
        </div>
        ...
        <svg width={200} height={80} viewBox="0 0 200 80" style={{ overflow: 'visible' }}>
          ...
          <text x={100} y={28} textAnchor="middle" fill={qualityColor(s2l?.quality)}
            fontSize={9} fontFamily="JetBrains Mono, monospace">
            {qualityLabel(s2l?.quality)} ({pct(s2l?.score)})
          </text>

          {/* arrow left */}
          <polygon points="20,46 5,40 20,34" fill={qualityColor(l2s?.quality)} opacity={0.8} />
          <text x={100} y={60} textAnchor="middle" fill={qualityColor(l2s?.quality)}
            fontSize={9} fontFamily="JetBrains Mono, monospace">
            {qualityLabel(l2s?.quality)} ({pct(l2s?.score)})
          </text>
        </svg>
        ...
        <div style={{ textAlign: 'center' }}>
          <div style={{
            width: 60, height: 60, borderRadius: '50%', border: `2px solid ${C.red}`,
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            fontSize: 24, background: `${C.red}15`, margin: '0 auto',
          }}>
            🔴
          </div>
          <div style={{ fontSize: 9, fontFamily: 'JetBrains Mono, monospace', color: C.red, marginTop: 6 }}>
            LISTENER
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'center', gap: 40, marginTop: 24, flexWrap: 'wrap' }}>
        <div>
          <CircularGauge value={trust} size={100} strokeWidth={8}
            color={trust > 0.5 ? C.green : trust > 0.25 ? C.amber : C.red}
            label="TRUST" sublabel="LEVEL" animate />
          <div style={{ fontSize: 9, fontFamily: 'JetBrains Mono, monospace', color: C.dim, marginTop: 6 }}>
            TRUST LEVEL
          </div>
        </div>
        <div>
          <CircularGauge value={compat} size={100} strokeWidth={8}
            color={compat > 0.5 ? C.green : compat > 0.25 ? C.amber : C.red}
            label="COMPAT" sublabel="IBILITY" />
          <div style={{ fontSize: 9, fontFamily: 'JetBrains Mono, monospace', color: C.dim, marginTop: 6 }}>
            COMPATIBILITY
          </div>
        </div>
...
function EmotionSummary({ speaker, listener }) {
  ...
  const emotionColors = {
    FRUSTRATED: C.red, EXCITED: C.amber, CURIOUS: C.cyan, FOCUSED: C.green,
    CONFUSED: '#aa66ff', SATISFIED: C.green, NEUTRAL: C.dim,
  };

  return (
    <div style={{
      background: C.panel, border: `1px solid ${C.dim}22`, borderRadius: 10,
      padding: 16, animation: 'am-fadeIn 0.6s ease-out',
    }}>
      <div style={{
        fontSize: 9, fontFamily: 'JetBrains Mono, monospace', color: C.dim,
        ...
        {entries.map(([emo, val]) => {
          ...
          return (
            <div key={emo} style={{ flex: 1, textAlign: 'center' }}>
              <div style={{
                height: `${h}%`, minHeight: 4, background: `linear-gradient(180deg, ${col}, ${col}66)`,
                borderRadius: '4px 4px 0 0', transition: 'height 0.6s ease',
              }} />
              <div style={{ fontSize: 7, fontFamily: 'JetBrains Mono, monospace', color: C.dim, marginTop: 4 }}>
                {emo.slice(0, 6)}
              </div>
            </div>
          );
  ...
  if (!sessionId) {
    return (
      <div style={{
        ...
        <div style={{ color: C.dim, fontSize: 14, letterSpacing: 2 }}>
          SELECT A SESSION TO SCAN AGENT MINDS
        </div>
      </div>
    );
  }
  ...
  if (loading) {
    return (
      <div style={{
        ...
        <div style={{
          fontSize: 32, animation: 'am-pulse 1.5s ease-in-out infinite', marginBottom: 16,
        }}>
          🧠
        </div>
        <div style={{ color: C.green, fontSize: 13, letterSpacing: 2, animation: 'am-blink 1.5s ease-in-out infinite' }}>
          SCANNING AGENT MINDS...
        </div>
  ...
  if (error) {
    return (
      <div style={{
        ...
        <div style={{ color: C.red, fontSize: 13, letterSpacing: 1 }}>
          MIND SCAN FAILED
        </div>
...
function normalizeMindsData(raw) {
  ...
  const normalizeAgent = (agent) => {
    ...
    return {
      ...
      current_emotion: {
        emotion: (emo.mood || 'neutral').toUpperCase(),
        emoji: emo.emoji || '😊',
        intensity: emo.energy ?? 0.5,
      },
      recent_thoughts: rel.thought ? [{
        type: 'strategy',
        content: rel.thought,
        episode: null,
      }] : [],
      recent_judgments: rel.judgment ? [{
        target: agent.agent_id === 'speaker' ? 'listener' : 'speaker',
        quality: rel.judgment.category || 'unknown',
        score: rel.judgment.score || 0,
        reason: rel.judgment.text || '',
      }] : [],
      memory: {
  ...
  // Build relationship from speaker's relationship data
  ...
  if (!data || (!data.speaker && !data.listener)) {
    return (
      <div style={{
        ...
        <div style={{ color: C.dim, fontSize: 13, letterSpacing: 1 }}>
          NO MIND DATA AVAILABLE
        </div>
        <div style={{ color: C.dim, fontSize: 10, marginTop: 8 }}>
  ...
  return (
    <div style={{
      ...
      <div style={{ marginBottom: 32, textAlign: 'center' }}>
        <div style={{
          fontSize: 24, fontWeight: 'bold', color: C.green, letterSpacing: 4,
          textShadow: `0 0 20px ${C.green}44`,
        }}>
          🧠 ◆ AGENT MINDS
        </div>
        <div style={{ fontSize: 11, color: C.dim, marginTop: 6, letterSpacing: 2 }}>
          How agents think, feel, and judge
        </div>
        <div style={{
          width: 200, height: 1, margin: '12px auto 0',
          background: `linear-gradient(90deg, transparent, ${C.green}44, transparent)`,
        }} />
      </div>

      {/* ── 1. Consciousness Stream (NEW) ── */}
      <ConsciousnessStream speaker={speaker} listener={listener} />

      {/* ── 2. Emotion Oscilloscope (NEW) ── */}
      <EmotionOscilloscope speaker={speaker} listener={listener} />

      {/* ── Agent Profile Cards (with Personality Evolution) ── */}
      <div style={{ display: 'flex', gap: 20, marginBottom: 28, flexWrap: 'wrap' }}>
        <AgentCard agent={speaker} name="SPEAKER" icon="🔵" color={C.cyan} />
        <AgentCard agent={listener} name="LISTENER" icon="🔴" color={C.red} />
      </div>

      {/* ── 4. Relationship Meter (NEW) ── */}
      ...
      <div style={{ marginBottom: 28 }}>
        <div style={{
          fontSize: 12, color: C.amber, letterSpacing: 3, marginBottom: 14,
          borderBottom: `1px solid ${C.amber}22`, paddingBottom: 8,
        }}>
          ◈ THINKING PROCESS FEED ◈
        </div>
        {allThoughts.length === 0 ? (
          <div style={{ color: C.dim, fontSize: 11, padding: 20, textAlign: 'center' }}>
            No recent thoughts recorded
          </div>
        ) : (
          <div style={{ maxHeight: 300, overflowY: 'auto', paddingRight: 4 }}>
            {allThoughts.map((t, i) => (
              <ThoughtBubble key={i} thought={t}
                agentColor={t.agent === 'speaker' ? C.cyan : C.red} />
            ))}
          </div>
        )}
      </div>
      ...
      <div style={{ marginBottom: 28 }}>
        <div style={{
          fontSize: 12, color: C.red, letterSpacing: 3, marginBottom: 14,
          borderBottom: `1px solid ${C.red}22`, paddingBottom: 8,
        }}>
          ◈ JUDGMENT PANEL ◈
        </div>
        <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
          <JudgmentCard
            judgment={speaker?.recent_judgments?.[0]}
            judgeColor={C.cyan}
            judgeLabel="SPEAKER"
          />
          <JudgmentCard
            judgment={listener?.recent_judgments?.[0]}
            judgeColor={C.red}
            judgeLabel="LISTENER"
          />
        </div>
